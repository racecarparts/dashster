...
func Weather() []model.SimpleWeather {
	...
	for i, location := range wCfg.Locations {
		resp, err := getRequestWithBackoff(fmt.Sprintf(wCfg.WeatherDataUrl, url.QueryEscape(location.Location)))
		if err != nil {
		...
		if err != nil {
			simpleWeatherReports[i].Location = "Problem unmarshalling weather report data: " + err.Error()
			continue
		}
